...
def generate_candidate_keyword_scores(phrase_list, word_score, min_keyword_frequency=1,keyword_candidates={}):
    # keyword_candidates = {}
...
class PyRake3(object):
    _model_file_name = "key_candidates.pickle"
    storage_dir = "../storage/"#settings.AI['storage']
    ...
    def _model_path(self):
        return self.storage_dir + self._model_file_name

    def get_key_words(self, sentence_list):
        # print("getkey")
        stop_words_pattern = build_stop_word_regex(self.__stop_words_list)
        key_db = self.load(self._model_path())
        # print("key",key_db)
        if key_db == None:
            key_db = {}
        phrase_list = generate_candidate_keywords(sentence_list, stop_words_pattern, self.__stop_words_list,
        ...
        keyword_candidates = generate_candidate_keyword_scores(phrase_list, word_scores, self.__min_keyword_frequency,key_db)

        sorted_keywords = sorted(six.iteritems(keyword_candidates), key=operator.itemgetter(1), reverse=True)
        self.dump(keyword_candidates,self._model_path())
        ...
        #
        #     rake = PyRake3("SmartStoplist.txt")
